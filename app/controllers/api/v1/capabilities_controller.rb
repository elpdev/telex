  ...
  def show
    render_data({
      ...
      resources: {
        ...
        albums: %w[index create show update destroy],
        folders: %w[index create show update destroy],
        notes: %w[index create show update destroy tree],
        ...
        tasks: %w[workspace projects boards cards],
      ...
      filters: {
        ...
        calendars: %w[updated_since],
        calendar_events: %w[calendar_id starts_from ends_to status source uid updated_since],
        calendar_occurrences: %w[calendar_id calendar_ids starts_from ends_to],
        contacts: %w[contact_type q updated_since],
        domains: %w[active],
        inboxes: %w[domain_id active pipeline_key],
        albums: [],
        folders: %w[parent_id source provider updated_since],
        notes: %w[folder_id updated_since],
        files: %w[folder_id source provider mime_type updated_since],
        messages: %w[inbox_id conversation_id status subaddress mailbox label_id q sender recipient received_from received_to updated_since],
        conversations: %w[inbox_id mailbox label_id q],
        outbound_messages: %w[domain_id conversation_id source_message_id status updated_since],
        tasks_projects: %w[updated_since],
        tasks_cards: %w[updated_since],
