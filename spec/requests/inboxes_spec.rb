...
RSpec.describe "Inboxes", type: :request do
  ...
  describe "GET /" do
    ...
    it "shows unread messages before newer read messages in inbox view by default" do
      user = create(:user)
      login_user(user)
      inbox = create(:inbox, local_part: "leo")
      ...
      expect(response.body).to include("sort:</span><span>unread</span>")
    end

    it "supports oldest-first inbox sorting" do
      user = create(:user)
      login_user(user)
      inbox = create(:inbox, local_part: "leo")
      create(:message, inbox: inbox, subject: "Newest", received_at: 1.minute.ago)
      create(:message, inbox: inbox, subject: "Oldest", received_at: 2.minutes.ago)

      get root_path, params: {sort: "oldest"}

      expect(response.body.index("Oldest")).to be < response.body.index("Newest")
      expect(response.body).to include("sort:</span><span>oldest</span>")
    end
